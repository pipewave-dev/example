...
func getConfig() configprovider.ConfigStore {
	return configprovider.FromGoStruct(
		pipewave.ConfigEnv{
			Env:     "pwecho",
			PodName: "pod-local", // for loging purpose, in real app, you can set it to os.Hostname() or get it from env var injected by k8s downward API
			Version: "0.0.1",     // for loging purpose
			...
			Valkey: configprovider.ValkeyT{
				PrimaryAddress: "localhost:29103",
				ReplicaAddress: "localhost:29103",
...
func inspectToken(ctx context.Context, token string, headers http.Header) (username string, isAnonymous bool, metadata map[string]string, err error) {
	// For demo purpose: token is username, but in real app, you should inspect token to get real userID
	return trimToken(token), false, nil, nil
}
