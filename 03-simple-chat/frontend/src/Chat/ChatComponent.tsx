...
export function Chat({ toUserId }: { toUserId: string }) {
    ...
    usePipewaveMessage(MSG_TYPE_CHAT_INCOMING_MSG, async (data: Uint8Array, id: string) => {
    ...
    usePipewaveMessage(MSG_TYPE_CHAT_USER_TYPING, async (data: Uint8Array) => {
    ...
    usePipewaveMessage(MSG_TYPE_CHAT_ACK, async (_: Uint8Array, id: string) => {
        setToUserOffline(false)
        setMessages(prev => prev.map(msg => msg.id === id ? { ...msg, status: 'sent' } : msg))
    })

    usePipewaveMessage(MSG_TYPE_ECHO_RESPONSE, async (data: Uint8Array, id: string) => {
    ...
    usePipewaveMessage(MSG_TYPE_CHAT_FAIL, async (_: Uint8Array, id: string) => {
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #eee', paddingBottom: 12 }}>
                <div>
                    ...
                    <div style={{ fontSize: '0.85rem', color: '#666' }}>
                        Status: <span style={{ color: isConnected ? 'green' : 'red', fontWeight: 'bold' }}>{status}</span>
                        {isSuspended && <button onClick={resetRetryCount} style={{ marginLeft: 8 }}>Retry</button>}
                    </div>
                </div>
            </div>
